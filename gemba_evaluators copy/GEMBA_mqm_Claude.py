...
# Few-shot examples for GEMBA MQM
few_shots = {
    "ende": {
            "source_lang": "English",
...
""",
    ...
    "encs": {
            ...
            "target_lang": "Czech",
            "target_seg": "Ve Vídni se ve Vídni obnovily rozhovory o oživení jaderného paktu, přičemž obě partaje se snaží posoudit vyhlídky na úspěch po posledních výměnách v jedn��ních.",
            "answer": """Critical:
no-error
Major:
...
# Folder paths
source_folder = "/Users/cameronpattison/Library/CloudStorage/OneDrive-Vanderbilt/For Publication/Machine Translation Article/Translation_stuff/Source_Text"
translation_folder = "/Users/cameronpattison/Library/CloudStorage/OneDrive-Vanderbilt/For Publication/Machine Translation Article/Translation_stuff/Translations/claude_translations_11.14.24"

# OpenAI API key and client initialization
openai_api_key = "sk-proj-soxTXTrcUEXD3OSrM3PJT3BlbkFJ9PDOZCcf4GvU7vt1qxeM"
client = openai.OpenAI(api_key=openai_api_key)

def process_file_pair(source_file, translation_file):
    ...
    input_data = {
        "source_lang": "Ancient Greek",
        "source_seg": source_text,
        "target_lang": "English",
        "target_seg": translation_text
    }

    prompts = apply_template(TEMPLATE_GEMBA_MQM, input_data)
    ...
    mqm_result = parse_mqm_answer(evaluation_result, list_mqm_errors=True, full_desc=True)
...
def main():
    ...
    for filename in sorted(os.listdir(source_folder)):  # Removed [:3] limit
        if filename.endswith(".txt"):
            source_file = os.path.join(source_folder, filename)
            translation_file = os.path.join(translation_folder, filename)

            if os.path.exists(translation_file):
                print(f"Processing {filename}...")
                result = process_file_pair(source_file, translation_file)
                results[filename] = result
            else:
                print(f"Warning: No matching translation file for {filename}")

    # Save results to a JSON file
    with open('claude_evaluation_results.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    print("Evaluation complete. Results saved to claude_evaluation_results.json")
